...
def iniciar_progresso() -> None:
    if _root_ref_for_log and _root_ref_for_log.winfo_exists() and progress and status_label:
        progress.start(20)
...
def parar_progresso(final_status: str = "") -> None:
    if _root_ref_for_log and _root_ref_for_log.winfo_exists() and progress and status_label:
        progress.stop()
        status_label.config(text=final_status or "Pronto.")
        _root_ref_for_log.update_idletasks()
...
# A PARTIÇÃO AINDA É ÚTIL PARA ORGANIZAR O SCHEMA, MAS NÃO SERÁ USADA PARA FAZER MÚLTIPLAS CHAMADAS
...
def gerar_particoes_dinamicamente() -> bool:
    ...
    log_to_gui(f"Partições geradas para organização: {len(LISTA_DE_NOMES_BLOCOS_PARTICIONADA)} ativas.", "INFO")
    ...
    try:
        with pdfplumber.open(caminho_pdf) as pdf:
            ...
            for i, pagina in enumerate(pdf.pages):
                if _root_ref_for_log and _root_ref_for_log.winfo_exists() and status_label:
...
def processar_pdf_e_gerar_saidas(caminho_pdf_path_obj: Path) -> bool:
    ...
    # --- INÍCIO DA REESTRUTURAÇÃO PARA CHAMADA ÚNICA DE API ---
    # Em vez de fazer um loop sobre as partições e chamar a API várias vezes,
    # agora faremos uma única chamada, enviando o BLOCO_CONFIG completo.
    # Isso é mais eficiente, mais barato e resolve o erro 429 (Too Many Requests / Tokens) com modelos maiores como o Gemini Pro.

    log_to_gui(f"Enviando todos os {len(BLOCO_CONFIG)} blocos do schema em uma única requisição para a API...", "INFO")
    if _root_ref_for_log and _root_ref_for_log.winfo_exists():
        status_label.config(text=f"Processando PDF com IA ({len(BLOCO_CONFIG)} blocos)...")
        _root_ref_for_log.update_idletasks()

    # A única e principal chamada para a API
    resultado_final_combinado_ia = enviar_texto_completo_para_gemini_todos_blocos(
        texto_completo_do_pdf=texto_completo_extraido,
        blocos_config_map=BLOCO_CONFIG,  # <<< A MUDANÇA PRINCIPAL: Usando o config completo
        pdf_path_para_logs=caminho_pdf_path_obj
    )

    # Verificação de erro para a chamada única
    if not resultado_final_combinado_ia:
        log_to_gui("ERRO FATAL: Falha ao processar com a API ou resultado inválido.", "CRITICAL")
        if _root_ref_for_log and _root_ref_for_log.winfo_exists():
            parar_progresso("Erro no processamento da API.")
            messagebox.showerror("Erro de API", "A chamada para a API falhou ou retornou dados vazios. Verifique os logs.", parent=_root_ref_for_log)
        return False
    
    # --- FIM DA REESTRUTURAÇÃO ---
    
    log_to_gui("API: Processamento concluído com sucesso.", "INFO")
    if CRIAR_ARQUIVOS_DEBUG_INTERMEDIARIOS:
    ...
    if _root_ref_for_log and _root_ref_for_log.winfo_exists(): status_label.config(text="Achatando JSON da IA..."); _root_ref_for_log.update_idletasks()
    json_achatado_da_ia = achatar_json(resultado_final_combinado_ia)
    
    if not json_achatado_da_ia and resultado_final_combinado_ia:
        if _root_ref_for_log and _root_ref_for_log.winfo_exists(): parar_progresso(f"Erro: JSON achatado da IA vazio para {caminho_pdf_path_obj.name}")
        log_to_gui("ERRO: Falha ao achatar JSON da API. Resultado vazio.", "ERROR")
        if _root_ref_for_log and _root_ref_for_log.winfo_exists(): messagebox.showerror("Erro Processamento", "O JSON da IA achatado resultou vazio.", parent=_root_ref_for_log)
        return False
    ...
    if json_achatado_da_ia:
        ...
        if not json_final_norm and json_achatado_da_ia:
            ...
            log_to_gui("ERRO: Falha ao normalizar chaves JSON.", "ERROR")
        ...
        if caminho_template_str:
            ...
            if caminho_saida_str:
                ...
                if not preencher_excel_novo_com_placeholders(nome_json_saida_excel, caminho_template, caminho_saida, nome_plan_alvo):
                    log_to_gui(f"Falha ao preencher Excel '{caminho_saida.name}'.", "ERROR")
            else:
                if _root_ref_for_log and _root_ref_for_log.winfo_exists(): parar_progresso("Salvamento do Excel cancelado.")
        else:
            if _root_ref_for_log and _root_ref_for_log.winfo_exists(): parar_progresso("Seleção de modelo Excel cancelada.")
    else:
        if _root_ref_for_log and _root_ref_for_log.winfo_exists(): parar_progresso(f"Falha ao gerar JSON para Excel ({caminho_pdf_path_obj.name})")
...
if __name__ == "__main__":
    ...
    if schema_ok:
        if BLOCO_CONFIG:
            # A geração de partições ainda é útil para LOG e validação do schema.
